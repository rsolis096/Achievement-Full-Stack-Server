...
export const getOwnedGames = async (req, res) => {
    try {
        ...
        const response = await axios.get(getOwnedAppsURL);
        const gamesFromAPI = response.data.response.games;
...
export const getGamesSearch = async (req, res) => {
    try {
        const lookupTerm = req.body.lookup;
        //Attempt to get User Game Library from database
